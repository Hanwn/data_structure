...
public:
	Udg_link(){};
	Udg_link(char v[],int v_size,char e[][2],int e_size);
	~Udg_link();
public:
	void print();
	char get_char();
...
Udg_link::Udg_link(char v[],int v_size,char e[][2],int e_size) {
	vlen = v_size;
	elen = e_size;
...
int Udg_link::get_position(char c) {
	for (int i = 0; i < vlen; i++) {
		if (c == vnode[i].data) {
			return i;
		}
...
void Udg_link::link_last(ENode* link,ENode* node) {
	ENode* p = link;
	while (p->next) {
		p = p->next;
	}
